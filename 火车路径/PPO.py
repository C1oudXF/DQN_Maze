import torch
from torch import nn  # torch.nn包内就包含了高级封装,里面包括了卷积,池化,RNN等计算,以及其他如Loss计算
import torch.nn.functional as F  # 包括一些函数，激活函数，规范化函数，Dropout 函数等
import numpy as np
...
def main():
    agent = PPO(num_state=4,num_action=4)
    env = Maze()
        ...
        for t in range(episode_step):
            ...
            if done:
                ...
                rospy.loginfo('Ep: %d score: %.2f memory: %d episode_step: %.2f time: %d:%02d:%02d', e,
                              episode_reward_sum, agent.counter, episode_step, h, m, s)
                break
